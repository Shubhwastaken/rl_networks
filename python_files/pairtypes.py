...
MODE = os.environ.get("MODE","BOTH")   # NN = singletons only, PP = |P|>=2 only, BOTH
def allowed(P):
    if MODE=="NN": return len(P)==1
    if MODE=="PP": return len(P)>=2
    return True
            ...
            if base[0] <= S[0] and base[1] <= S[1]:
                ...
                if T in Cset and T != S:
                    eq([(idx[T], 1.0), (idx[S], -1.0)], 0.0,
                       f"SETDEC|{len(P)}| P={{{','.join(P)}}}  {nm(T)} = {nm(S)}")
...
print(f"MODE={MODE}  PRIMAL BOUND  r <= {BOUND:.10f}\n")
...
mix=_c.Counter()
for m,t in chain:
    g=_re.match(r"SETDEC\|(\d+)\|",t)
    if g: mix[int(g.group(1))]+=1
print(f"  chain length = {len(chain)}   node-set sizes used: {dict(sorted(mix.items()))}")
