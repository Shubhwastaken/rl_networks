...
NSUB = int(os.environ.get("NSUB","100000"))
...
def conn(P):
    ...
    st=[next(iter(P))]; seen={st[0]}
    while st:
        x=st.pop()
        for y in adj[x]:
            if y in P and y not in seen: seen.add(y); st.append(y)
    return seen==P
cands=[]
for kk in range(1, len(nodes)):
    for P in itertools.combinations(nodes, kk):
        ...
        if not dec: continue
        comp=[x for x in nodes if x not in set(P)]
        if len(nodes)>10 and not (conn(P) and conn(comp)): continue
        cands.append((len(In(P)), P, dec))
cands.sort()
keep=cands[:NSUB]
for _,P,dec in keep:
    cut = In(P); sp = Src(P)
    add([], cut); add(sp, cut); add(sp + dec, cut)
# targeted: pairwise intersections / unions of the in-arc sets of kept regions,
# and the partial session-accumulations that a CHAIN would pass through
if os.environ.get("TARGETED","0")=="1":
    for i in range(len(keep)):
        for j in range(i+1,len(keep)):
            _,P1,d1 = keep[i]; _,P2,d2 = keep[j]
            A1,A2 = frozenset(In(P1)), frozenset(In(P2))
            s1,s2 = frozenset(Src(P1)), frozenset(Src(P2))
            for A in (A1|A2, A1&A2):
                if not A: continue
                add([], A); add(s1|s2, A)
                add((s1|s2)|frozenset(d1), A)
                add((s1|s2)|frozenset(d1)|frozenset(d2), A)
if BIG:
